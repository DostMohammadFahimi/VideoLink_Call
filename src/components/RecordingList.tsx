    ...
    <div className="space-y-3 text-center">
      ...
      <ul className="list-inside list-disc">
        {recordings
          ...
          .map((recordings) => (
            <li key={recordings.url}>
              <Link
                href={recordings.url}
                target="_blank"
                className="hover:underline"
              >
                {new Date(recordings.end_time).toLocaleString()}
              </Link>
            </li>
          ))}
      </ul>
      <p className="text-sm text-gray-500">
          <span className="font-xl font-bold">Note:</span> It can take up to 1 minute before new recordings show up.
          <br />
